...
const AnimatedProjectCard = ({ p }) => {
  ...
  return (
    <div className="relative group bg-slate-900/50 backdrop-blur-sm border border-slate-800 p-6 rounded-3xl overflow-hidden transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_0_40px_-10px_rgba(139,92,246,0.3)] hover:border-violet-500/50">
      ...
      <h3 className="relative z-10 font-bold text-slate-100 text-xl mb-3 group-hover:text-violet-300 transition-colors">{p?.title || "Untitled Project"}</h3>
      
      <div className="relative z-10 flex-1 mb-6">
        <p className={`text-slate-400 text-sm leading-relaxed transition-all duration-300 ${expanded ? '' : 'line-clamp-3'}`}>
      ...
      <div className="relative z-10 flex gap-3 mt-auto pt-4 border-t border-slate-800/50">
        {p?.github && (
          <a href={p.github} target="_blank" rel="noreferrer" className="flex-1 text-center bg-slate-800 hover:bg-slate-700 text-slate-200 py-2.5 rounded-xl text-sm font-semibold transition-all hover:shadow-lg">
            Source Code
          </a>
        )}
        {p?.live && (
          <a href={p.live} target="_blank" rel="noreferrer" className="flex-1 text-center bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:from-violet-500 hover:to-fuchsia-500 text-white py-2.5 rounded-xl text-sm font-semibold transition-all shadow-[0_0_15px_rgba(139,92,246,0.4)] hover:shadow-[0_0_25px_rgba(139,92,246,0.6)]">
            Live Preview
          </a>
        )}
      </div>
...
export default function Template4Animated({ data }) {
  ...
  return (
    <div className="relative bg-slate-950 text-slate-300 font-sans min-h-screen pb-10 selection:bg-violet-500/30 selection:text-violet-200 overflow-hidden">
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-6 md:px-12 pt-40">
        ...
        <div id="hero" className="scroll-mt-40 flex flex-col lg:flex-row items-center justify-between gap-16 min-h-[70vh]">
          ...
          <div className="flex-1 space-y-8 z-10 text-center lg:text-left">
            ...
            <h2 className="text-xl md:text-3xl text-slate-400 font-medium">
              I'm a <span className="text-white border-b-2 border-violet-500">{data?.title || "Software Developer"}</span>
            </h2>

            <p className="text-slate-400 text-lg md:text-xl max-w-xl leading-relaxed line-clamp-3 mx-auto lg:mx-0" title={data?.bio}>
              {data?.bio || "Building next-generation applications with a focus on seamless user experiences and scalable architecture."}
            </p>
            ...
            {hasProjects &&
              ...
              {data?.resume && (
                <a href={data.resume} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-8 py-4 border-2 border-slate-700 text-slate-300 rounded-2xl font-bold hover:bg-slate-800 hover:text-white transition-all hover:border-slate-600">
            ...
            <div className="flex justify-center lg:justify-start gap-5 pt-6">
               {data?.github && (
                  <a href={data.github} target="_blank" rel="noreferrer" className="w-12 h-12 rounded-xl bg-slate-800/80 border border-slate-700 flex items-center justify-center text-slate-400 hover:text-white hover:bg-violet-600 hover:border-violet-500 hover:-translate-y-2 transition-all duration-300 shadow-lg">
               ...
               {data?.linkedin && (
                  <a href={data.linkedin} target="_blank" rel="noreferrer" className="w-12 h-12 rounded-xl bg-slate-800/80 border border-slate-700 flex items-center justify-center text-slate-400 hover:text-white hover:bg-blue-600 hover:border-blue-500 hover:-translate-y-2 transition-all duration-300 shadow-lg">
               ...
               {data?.email && (
                  <a href={`mailto:${data.email}`} className="w-12 h-12 rounded-xl bg-slate-800/80 border border-slate-700 flex items-center justify-center text-slate-400 hover:text-white hover:bg-rose-500 hover:border-rose-400 hover:-translate-y-2 transition-all duration-300 shadow-lg">
        ...
        {/* ABOUT */}
        {hasAbout && (
           <div id="about" className="scroll-mt-40 py-20 relative">
        ...
        {hasExperience && (
           <div id="experience" className="scroll-mt-40 py-20">
             ...
             <div className="max-w-4xl mx-auto space-y-6">
               {data.experience.map((exp, i) => (
                 ...
                 >
                   ...
                   <div className="md:w-1/3 flex flex-col justify-start md:border-r border-slate-800 pr-6 relative z-10">
                      ...
                      <h3 className="font-bold text-xl text-slate-100 mb-1 group-hover:text-violet-300 transition-colors">{exp.company}</h3>
        ...
        {/* PROJECTS */}
        {hasProjects && (
           <div id="projects" className="scroll-mt-40 py-20 relative">
        ...
        {hasSkills && (
           <div id="skills" className="scroll-mt-40 py-20">
             <div className="text-center mb-16">
        ...
        {/* CONTACT SECTION */}
        <div id="contact" className="scroll-mt-40 py-20 mb-20 relative">
          ...
          <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-700/50 rounded-[3rem] p-8 md:p-16 shadow-[0_20px_50px_rgba(0,0,0,0.5)] max-w-5xl mx-auto relative overflow-hidden">
             ...
             <div className="grid lg:grid-cols-2 gap-16 relative z-10">
                ...
                {/* Right: Form */}
                <form suppressHydrationWarning className="bg-slate-950/50 p-8 rounded-3xl border border-slate-800/80 space-y-5">
                   <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-wider pl-1">Your Name</label>
                      <input placeholder="John Doe" className="w-full bg-slate-900 border border-slate-700 text-white p-4 rounded-xl focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 transition-all" />
                   </div>
                   <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-wider pl-1">Your Email</label>
                      <input placeholder="john@example.com" type="email" className="w-full bg-slate-900 border border-slate-700 text-white p-4 rounded-xl focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 transition-all" />
                   </div>
                   <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-wider pl-1">Message</label>
                      <textarea placeholder="Tell me about your project..." rows="4" className="w-full bg-slate-900 border border-slate-700 text-white p-4 rounded-xl focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 transition-all resize-none"></textarea>
                   </div>
                   <button type="button" className="w-full bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:from-violet-500 hover:to-fuchsia-500 text-white font-bold py-4 rounded-xl transition-all shadow-[0_10px_20px_rgba(139,92,246,0.3)] hover:shadow-[0_15px_30px_rgba(139,92,246,0.5)] hover:-translate-y-1">
                     Send Message
                   </button>
                </form>
