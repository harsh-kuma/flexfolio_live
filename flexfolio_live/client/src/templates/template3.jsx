  ...
  return (
    <div className="bg-neutral-900 border border-neutral-800 hover:border-emerald-500/50 p-6 rounded-2xl shadow-lg hover:-translate-y-1 transition-all flex flex-col group relative overflow-hidden">
      ...
      <div className="flex justify-between items-start mb-4 relative z-10">
        <h3 className="font-bold text-neutral-100 text-xl line-clamp-1" title={p?.title}>{p?.title || "Untitled Project"}</h3>
        {p?.year && (
      ...
      <div className="flex-1 mb-5 relative z-10">
        <p className={`text-neutral-400 text-sm leading-relaxed transition-all duration-300 ${expanded ? '' : 'line-clamp-3'}`}>
...
export default function Template3({ data }) {
  ...
  return (
    <div className="bg-neutral-950 text-neutral-300 font-sans min-h-screen pb-10 selection:bg-emerald-500/30 selection:text-emerald-200">
      ...
      <nav className="fixed top-0 left-0 w-full z-[60] bg-neutral-950/90 backdrop-blur-md border-b border-neutral-800/80">
        <div className="max-w-6xl mx-auto px-6 md:px-12 py-4 flex justify-between items-center relative z-[60]">
          ...
          <button 
            className="md:hidden text-neutral-300 hover:text-emerald-400 focus:outline-none p-1 transition-colors" 
      ...
      <main className="max-w-5xl mx-auto px-6 md:px-12 pt-32">
        ...
        <div id="hero" className="scroll-mt-32 flex flex-col items-center text-center py-10 md:py-16">
          <div className="relative mb-8">
            ...
            <img
              ...
              alt={data?.fullName}
              className="w-32 h-32 md:w-40 md:h-40 rounded-full object-cover shadow-2xl border-2 border-neutral-800 relative z-10"
            />
          </div>
          ...
          <h2 className="text-xl md:text-2xl text-emerald-400 font-mono mb-6 flex items-center gap-2">
            <span className="text-neutral-600">&gt;</span> {data?.title || "Software Developer"} <span className="animate-pulse">_</span>
          </h2>

          <p className="text-neutral-400 text-lg max-w-2xl leading-relaxed mb-8 line-clamp-3" title={data?.bio}>
            {data?.bio || "Transforming ideas into elegant, scalable, and efficient digital solutions."}
          </p>
        ...
        <div id="contact" className="scroll-mt-24 py-16 max-w-3xl mx-auto">
          <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-8 md:p-12 shadow-2xl">
             ...
             <form suppressHydrationWarning className="flex flex-col gap-5">
                <div className="grid md:grid-cols-2 gap-5">
                   <div className="flex flex-col gap-2">
                      <label className="text-xs font-mono text-emerald-400 uppercase tracking-wider">Name</label>
                      <input placeholder="Your Name" className="w-full bg-neutral-950 border border-neutral-800 text-neutral-200 p-3.5 rounded-xl focus:outline-none focus:border-emerald-500 transition-colors" />
                   </div>
                   <div className="flex flex-col gap-2">
                      <label className="text-xs font-mono text-emerald-400 uppercase tracking-wider">Email</label>
                      <input placeholder="your@email.com" type="email" className="w-full bg-neutral-950 border border-neutral-800 text-neutral-200 p-3.5 rounded-xl focus:outline-none focus:border-emerald-500 transition-colors" />
                   </div>
                </div>
                <div className="flex flex-col gap-2">
                   <label className="text-xs font-mono text-emerald-400 uppercase tracking-wider">Message</label>
                   <textarea placeholder="How can we help each other?" rows="5" className="w-full bg-neutral-950 border border-neutral-800 text-neutral-200 p-3.5 rounded-xl focus:outline-none focus:border-emerald-500 transition-colors resize-none"></textarea>
                </div>
                <button type="button" className="bg-emerald-600 hover:bg-emerald-500 text-white font-medium py-4 rounded-xl transition-colors mt-2">
                  Send Message
                </button>
             </form>
          </div>
      ...
      <footer className="border-t border-neutral-800 mt-10">
        <div className="max-w-5xl mx-auto px-6 py-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-neutral-500 text-sm font-mono">
            © {new Date().getFullYear()} {data?.fullName}. Built with Flexfolio.
          </p>
          <div className="flex gap-4">
            <span className="text-neutral-600 text-sm">{data?.email}</span>
            {data?.email && data?.location && <span className="text-neutral-800">•</span>}
            <span className="text-neutral-600 text-sm">{data?.location}</span>
          </div>
        </div>
      </footer>
    </div>
