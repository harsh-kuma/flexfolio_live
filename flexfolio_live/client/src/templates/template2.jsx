...
const ProjectCard = ({ p }) => {
  ...
  return (
    <div className="bg-white border border-slate-100 p-5 rounded-2xl shadow-sm hover:shadow-xl transition-shadow flex flex-col group overflow-hidden">
      ...
      <h3 className="font-bold text-slate-800 text-lg mb-2 line-clamp-1" title={p.title}>{p.title}</h3>
      
      <div className="flex-1 mb-4">
        <p className={`text-slate-500 text-sm transition-all duration-300 ${expanded ? '' : 'line-clamp-3'}`}>
          {p.description}
        </p>
        {isLongText && (
          <button 
      ...
      <div className="flex gap-3 mt-auto">
        {p.github && (
          <a href={p.github} target="_blank" rel="noreferrer" className="flex-1 text-center border border-slate-200 text-slate-700 hover:bg-slate-50 hover:text-blue-600 py-2 rounded-lg text-sm font-medium transition-colors">
            View Code
          </a>
        )}
        {p.live && (
          <a href={p.live} target="_blank" rel="noreferrer" className="flex-1 text-center bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white py-2 rounded-lg text-sm font-medium transition-colors">
            Live App
          </a>
        )}
      </div>
        ...
        <div className="flex justify-between items-center relative z-50">
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => scrollTo("#hero")}>
            <div className="w-10 h-10 rounded-full border-2 border-blue-500 bg-transparent text-white flex items-center justify-center font-bold shrink-0">
              {getInitials(data?.fullName || "Portfolio")}
            </div>
            <div>
              <h1 className="font-bold text-white tracking-wide">{data?.fullName?.split(" ")[0]?.toUpperCase() || "PORTFOLIO"}</h1>
              <p className="text-[10px] tracking-wider text-slate-400">{data?.title || "Developer"}</p>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-6 md:px-12 pt-24">
        ...
        <div id="hero" className="scroll-mt-24 flex flex-col-reverse md:flex-row items-center justify-between gap-10 py-6 md:py-8">
          ...
          <div className="flex-1 space-y-5">
            ...
            <h1 className="text-2xl md:text-4xl font-extrabold text-slate-900 leading-tight">
              ...
              <span className="text-blue-600">{data?.title || "Full Stack Developer"}</span>
            </h1>
            ...
            <div className="flex flex-wrap gap-4 pt-2">
              ...
              {data?.resume && (
                <a href={data.resume} target="_blank" rel="noreferrer" className="border-2 border-slate-200 text-slate-700 hover:border-slate-300 hover:bg-slate-50 px-6 py-3 rounded-lg font-medium transition-colors flex items-center gap-2">
            ...
            <div className="pt-4">
              ...
              <div className="flex gap-4">
                {data?.linkedin && (
                  <a href={data.linkedin} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full bg-white shadow flex items-center justify-center text-blue-600 hover:-translate-y-1 transition-transform">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg>
                  </a>
                )}
                {data?.github && (
                  <a href={data.github} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full bg-white shadow flex items-center justify-center text-slate-800 hover:-translate-y-1 transition-transform">
                ...
                {data?.email && (
                  <a href={`mailto:${data.email}`} className="w-10 h-10 rounded-full bg-white shadow flex items-center justify-center text-red-500 hover:-translate-y-1 transition-transform">
        ...
        <div id="contact" className="scroll-mt-24 py-10">
          ...
          <div className="bg-white p-6 md:p-10 rounded-3xl shadow-sm border border-slate-100 grid md:grid-cols-2 gap-10 items-center">
            ...
            {/* Form */}
            <form suppressHydrationWarning className="flex flex-col gap-4 bg-slate-50 p-6 rounded-2xl border border-slate-100">
              <div className="grid grid-cols-2 gap-3">
                 <div className="flex flex-col gap-1">
                    <label className="text-xs font-medium text-slate-700">Your Name</label>
                    <input placeholder="John Doe" className="w-full border border-slate-200 p-2.5 text-sm rounded-xl focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
                 </div>
                 <div className="flex flex-col gap-1">
                    <label className="text-xs font-medium text-slate-700">Email</label>
                    <input placeholder="john@example.com" type="email" className="w-full border border-slate-200 p-2.5 text-sm rounded-xl focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
                 </div>
              </div>
              <div className="flex flex-col gap-1">
                 <label className="text-xs font-medium text-slate-700">Message</label>
                 <textarea placeholder="Write your message here..." rows="4" className="w-full border border-slate-200 p-2.5 text-sm rounded-xl focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 resize-none"></textarea>
              </div>
              <button type="button" className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium py-2.5 rounded-xl transition-colors mt-1 flex items-center justify-center gap-2">
